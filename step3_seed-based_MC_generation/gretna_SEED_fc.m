...
%       ROI_mask:
%                   The SEED ROI mask (if binary ROI mask, then equal-weighted
%                   mean time series; if weighted ROI mask (e.g.,
%                   propobility map), then unequal-weighted mean time
%                   series).
%       Brain_mask:
...
[Vmask, Ymask, ~]= gretna_read_image(Brain_mask);
Ymask(isnan(Ymask)) = 0;
...
[~, Yseed, ~] = gretna_read_image(ROI_mask);
Yseed(isnan(Yseed)) = 0;
...
for isub = 1:Num_subs
    ...
    TC_roi = TC_roi*Yseed(ind_seed)/length(ind_seed);           
    ...
    cd (Output_path)
    [~, name, ~] = fileparts(Dir_data{1}{isub});
    Vout.pinfo(1) = 1;
    Vout.fname = ['FC_' name  '.nii'];
    Vout = spm_write_vol(Vout,fc);
    
    Vout.fname = ['zFC_' name  '_Order_' num2str(isub, '%04d') '.nii'];
